...
class RowPreprocessor:
    standard_keys = ['messages', 'rejected_response', 'label', 'images', 'videos', 'audios', 'tools', 'objects', 'dialogue_id']
    ...
    def __call__(
        ...
        *,
        ...
        # if 'dialogue_id' in dataset.features:
        #     with safe_ddp_context(None, True):
        #         dataset = dataset.map(lambda x: {'__#dialogue_id': x['dialogue_id']}, **map_kwargs)
...
class ResponsePreprocessor(RowPreprocessor):
    ...
    def __init__(self, *, columns: Optional[Dict[str, str]] = None, **kwargs) -> None:
        ...
        response_keys = ['response', 'answer', 'output', 'targets', 'target', 'answer_key', 'answers', 'solution', 'dialogue_id'
